         ...
         <div class="row gap-y-12">
            ...
            <div class="col-md-6 md:pl-3.5">
               <div class="divide-y divide-base-content/55">
                  ...
                  <div class="pt-4 pb-8">
                     <span class="text-base text-base-content mb-4 block">{{$option->name}}</span>
                     <x-error class="mt-2 mb-4" :name="'selectedOptions.' . $option->id" />
                     <div class="flex items-center gap-2">
                        @foreach ($product->optionValues()->where('option_id', $option->id)->get() as $value)
                        <div class="radio-option">
                           <input type="radio"
                              name="selectedOptions[{{ $option->id }}]"
                              wire:model="selectedOptions.{{ $option->id }}"
                              wire:change="changedOptions()"
                              value="{{ $value->id }}"
                              id="product_option_{{$value->id}}">
                           <label for="product_option_{{$value->id}}">
                              {{$value->name}}
                           </label>
                        </div>
                        @endforeach
